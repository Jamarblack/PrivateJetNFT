...
function App() {
  ...
  const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyi-fpAlr-7Apr2BCkLHh94vrUc4eDfk20p9jYS7Ikqz6tlIKZBuMTLJuI36mFXjzrJvw/exec';
  ...
  const submitToSheet = async () => {
    ...
    formData.append('commentLink', commentLink);
    formData.append('walletAddr', walletAddr);

    try {
      await fetch(GOOGLE_SCRIPT_URL, {
        method: 'POST',
        body: formData,
        mode: 'no-cors' // Bypasses standard browser CORS blocks for Google Scripts
      });
      ...
      alert('Network error. Could not connect to the syndicate. Try again.');
    } finally {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
          <div className="relative bg-concrete border-2 border-neon-orange w-full max-w-2xl overflow-hidden shadow-[8px_8px_0px_#FF4500]">
            ...
            <div className="relative z-10 flex items-center justify-between px-6 pt-6">
              ...
              ) : <div />} {/* Empty div to keep alignment on success step */}
            ...
            <div className="relative z-10 p-8 md:p-12 pt-4">
              ...
              {step === 2 && (
                <div className="mt-8 animate-fade-in">
                  ...
                  <div className="text-xs text-white/30 font-mono mb-8 h-4">
                    {walletAddr.length > 0 && !isWalletValid ? <span className="text-neon-orange">must be 0x followed by 40 hex characters</span> : <span> EVM address</span>}
                  </div>
                  <button 
                    disabled={!isWalletValid || isSubmitting} 
                    onClick={submitToSheet} 
                    className={`w-full py-4 font-display text-xl tracking-widest transition-all ${isWalletValid && !isSubmitting ? 'bg-acid-yellow text-black shadow-[4px_4px_0px_#FF4500] hover:translate-y-1 hover:shadow-none' : 'bg-white/5 text-white/30 cursor-not-allowed'}`}
                  >
                    {isSubmitting ? 'PROCESSING...' : 'SIGN & SUBMIT TO SHEET'}
                  </button>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="mt-8 animate-fade-in text-center">
                  ...
                  <div className="font-display text-5xl text-acid-yellow tracking-widest mb-4">YOU ARE IN</div>
                  <p className="text-lg text-white/70 mb-8 font-mono">
                    Data secured. Wallet <span className="text-neon-orange">{walletAddr.slice(0, 6)}...{walletAddr.slice(-4)}</span> is registered for the takeover.
                  </p>
                  <button
                    onClick={closeModal}
                    className="w-full py-4 font-display text-xl tracking-widest transition-colors bg-white text-black hover:bg-neon-orange hover:text-white"
                  >
                    RETURN TO BASE ➔
                  </button>
                </div>
